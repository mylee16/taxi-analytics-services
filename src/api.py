...
def return_total_trip_of_day(start: str, end: str):
    ...
    total_trips = (
        data[["date", "unique_key"]][(data["date"] >= start) & (data["date"] <= end)]
        .groupby(["date"])
        .count()
        .reset_index()
...
def get_average_fare_heatmap(date: str):
    ...
    fare_heatmap = (
        data[["date", "s2id", "fare"]][(data["date"] == date) & (data["s2id"] != 0)]
        .groupby(["s2id"])
        .mean()
        .reset_index()
    )
...
def get_average_speed(date: str):
    ...
    total_distance_travelled_km = (
        data[["trip_miles"]][data["date"] == date].sum()[0] * 1.60934
    )
    total_time_taken_hours = (
        data[["trip_seconds"]][data["date"] == date].sum()[0] / 3600
    )
